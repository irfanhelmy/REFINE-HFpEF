...
merge m:m scrssn using "P:\ORD_Sundaram_202108013D\Padmini\Diabetes Stata Files\VS new files\dpp4\diabetes_hf_dpp4_dod_new.dta"
drop _merge
merge m:m scrssn using "P:\ORD_Sundaram_202108013D\Padmini\Diabetes Stata Files\diabetes_hf_sglt2_dodnew.dta"
drop if _merge ==3
drop _merge
merge m:m scrssn using "P:\ORD_Sundaram_202108013D\Padmini\Diabetes Stata Files\VS new files\sglt2\diabetes_hf_sglt2_dod_new.dta"
drop _merge
replace dod=MPI_DOD if dod==.
drop MPI_DOD
...
merge 1:1 scrssn using "P:\ORD_Sundaram_202108013D\Padmini\Diabetes Stata Files\VS new files\diabetes_hf_final_sglt2_dpp4_new.dta"
drop _merge
gen year= year(dof)
drop dod
...
merge 1:1 scrssn using "P:\ORD_Sundaram_202108013D\Padmini\Diabetes Stata Files\VS new files\dpp4\final_age_dof_new_itt.dta"
drop _merge
save "P:\ORD_Sundaram_202108013D\Padmini\Diabetes Stata Files\VS new files\final_survival_analysis_data_new_itt.dta", replace

*** STEP 7: Survival analyses 
clear all
use "P:\ORD_Sundaram_202108013D\Padmini\Diabetes Stata Files\VS new files\final_survival_analysis_data_new_itt.dta"
stset outcome, id(scrssn) origin(dof) failure(failure==1) exit(failure ==1 time td(31december2024)) scale(365.25)

stcox treatment agegp obesity sex AF copd depression alcohol hypertension CAD MI ckd cld cancer pad polyabuse ppm schizo stroke HFH THFH year weight2

stcox treatment agegp obesity sex AF copd depression alcohol hypertension CAD MI ckd cld cancer pad polyabuse ppm schizo stroke  HFH THFH year

sts graph, by(treatment) adjustfor(bmi agegp sex AF ckd cld copd HFH THFH CAD MI ACE BB antiarr insulin LD metformin spiro ppm weight2)